 ...
 * @param[in]  additional_terms Additional notmuch search terms
                             ...
                             const char *additional_terms)
{
  ...
  if (!mutt_str_equal(additional_terms, ""))
  {
    char *date_part = mutt_str_dup(buf);
    length = snprintf(buf, buflen, "(%s or (%s))", date_part, additional_terms);
    FREE(&date_part);
  }
